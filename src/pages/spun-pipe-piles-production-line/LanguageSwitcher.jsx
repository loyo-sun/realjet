...
const languages = [
  {
    ...
    label: "English",
    lang: "en",
    href: "/marketing/spun-pipe-piles-production-line/",
  },
  {
    code: "id",
    flag: "🇮🇩",
    ...
    lang: "id",
    href: "/marketing/spun-pipe-piles-production-line/id/",
  },
  {
    code: "vi",
    flag: "🇻🇳",
...
export default function LanguageSwitcher({ current = "en" }) {
  ...
  return (
    <details className="group relative shrink-0">
      <summary className="flex min-h-10 cursor-pointer list-none items-center gap-1.5 rounded-lg border border-white/15 px-2.5 text-[11px] font-[750] text-white/80 transition hover:border-white/30 hover:text-white [&::-webkit-details-marker]:hidden">
      ...
      <div className="absolute top-[calc(100%+8px)] right-0 z-50 min-w-[190px] overflow-hidden rounded-lg border border-white/10 bg-brand-navy p-1.5 shadow-floating">
        {languages.map(({ code, flag, label, lang, href }) => (
          <a
            ...
            className={`flex items-center gap-2 rounded-md px-3 py-2 text-[12px] no-underline transition ${
