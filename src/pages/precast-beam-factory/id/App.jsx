...
function Header({
  ...
  return <header className="sticky top-0 z-40 h-[70px] border-b border-white/10 bg-brand-navy/97 text-white backdrop-blur-xl max-[720px]:h-[62px]">
      <div className="site-container flex h-full items-center gap-6 max-[720px]:gap-2">
        <a href="/" aria-label="Beranda Realjet" className="shrink-0">
        ...
        <button onClick={() => onLead("Dapatkan Rancangan Lini Gratis")} className="rounded-lg bg-[#d94824] px-3.5 py-2 text-xs font-[850] text-white shadow-[0_8px_22px_rgba(217,72,36,.28)] transition hover:bg-[#b93619] focus-visible:bg-[#b93619] max-[1100px]:ml-auto max-[820px]:hidden">Dapatkan Rancangan Lini Gratis</button>
...
function LeadModal({
  ...
  const handleSubmit = async event => {
    ...
    const body = createBeamFactoryEnquiryBody(form, { locale: "id", title });
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#03111d]/75 p-5 backdrop-blur-lg" onMouseDown={event => event.target === event.currentTarget && onClose()}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="lead-title" className="relative max-h-[calc(100vh-40px)] w-full max-w-[680px] overflow-auto rounded-[18px] bg-white p-7 shadow-[0_30px_90px_rgba(0,0,0,.35)]">
        ...
        {submitted ? <div className="py-10 text-center">
            ...
            <p className="mt-1.5 mb-5 text-xs text-muted">Masukkan nama, e-mail, dan pesan Anda.</p>
            <form name={UNIVERSAL_ENQUIRY_FORM_NAME} method="POST" data-netlify="true" netlify-honeypot="bot-field" aria-busy={submissionState === "submitting"} onSubmit={handleSubmit}>
              ...
              <UniversalEnquiryFields locale="id" submissionState={submissionState} />
            </form>
          </>}
      </div>
    </div>;
