  ...
  return (
    <header className="sticky top-0 z-40 h-[70px] border-b border-white/10 bg-brand-navy/97 text-white backdrop-blur-xl max-[720px]:h-[62px]">
      <div className="site-container flex h-full items-center gap-6 max-[720px]:gap-2">
        ...
        <button onClick={() => onLead("Бесплатная схема линии под проект")} className="rounded-lg bg-[#d94824] px-3.5 py-2 text-xs font-[850] text-white shadow-[0_8px_22px_rgba(217,72,36,.28)] transition hover:bg-[#b93619] focus-visible:bg-[#b93619] max-[1100px]:ml-auto max-[720px]:hidden">Бесплатная схема линии под проект</button>
...
function LeadModal({ open, onClose, title }) {
  ...
  const handleSubmit = async (event) => {
    ...
    const body = createBeamFactoryEnquiryBody(form, { locale: "ru", title });
      ...
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="lead-title" className="relative max-h-[calc(100vh-40px)] w-full max-w-[680px] overflow-auto rounded-[18px] bg-white p-7 shadow-[0_30px_90px_rgba(0,0,0,.35)]">
        ...
        {submitted ? (
          ...
          <>
            ...
            <p className="mt-1.5 mb-5 text-xs text-muted">Укажите имя, e-mail и сообщение.</p>
            <form name={UNIVERSAL_ENQUIRY_FORM_NAME} method="POST" data-netlify="true" netlify-honeypot="bot-field" aria-busy={submissionState === "submitting"} onSubmit={handleSubmit}>
              ...
              <UniversalEnquiryFields locale="ru" submissionState={submissionState} />
            </form>
          </>
        )}
      </div>
  ...
  return (
    <>
      <div id="site-shell" data-locale="ru">
        ...
        <FloatingContactActions canonicalUrl="https://realjetech.com/marketing/precast-beam-factory/ru/" enquiryTitle="Бесплатная схема линии под проект" onEnquire={openLead} subject="линия производства сборных балок" />
