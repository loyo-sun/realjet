...
export default function FloatingContactActions({ canonicalUrl, enquiryTitle, onEnquire, subject }) {
  ...
  return (
    <nav className="fixed top-1/2 right-[18px] z-40 grid -translate-y-1/2 gap-2 rounded-[14px] border border-white/15 bg-[#051a2c]/90 p-2 shadow-[0_18px_46px_rgba(5,26,44,.28)] backdrop-blur-xl max-[720px]:right-[10px] max-[720px]:gap-1.5 max-[720px]:p-1.5" aria-label="Contact Realjet">
      {enquiryAction}
      <a href={`https://wa.me/8619310090600?text=${whatsappText}`} target="_blank" rel="noopener noreferrer" onClick={() => trackEvent("floating_contact_click", { channel: "whatsapp" })} className={`${actionClass} bg-white/8`} aria-label="WhatsApp" title="WhatsApp">
